...
async def criar_split_pagamento(split: SplitPagamento):
    async with AsyncClient() as client:
        response = await client.post(
            f'{BASE_URL}/paymentSplits',  # Endpoint para criar splits de pagamento
...
async def listar_splits_recebidos(offset: int = 0, limit: int = 100, paymentId: Optional[str] = None, status: Optional[str] = None):
    async with AsyncClient() as client:
        ...
        response.raise_for_status()
        return response.json()["data"]  # Retorna apenas os dados dos splits recebidos
...
async def listar_splits_pagos(offset: int = 0, limit: int = 100, paymentId: Optional[str] = None, status: Optional[str] = None):
    async with AsyncClient() as client:
        ...
        response.raise_for_status()
        return response.json()["data"]  # Retorna apenas os dados dos splits pagos


async def show_split_pagamento():
    ...
    with st.form(key='form_split_pagamento'):
        ...
        percentage = st.number_input("Percentual do Split (opcional)", min_value=0.0, max_value=100.0)

        submit_button = st.form_submit_button("Criar Split de Pagamento")

        if submit_button:
            split = SplitPagamento(paymentId=paymentId, walletId=walletId, value=value, percentage=percentage)
            try:
                resultado = await criar_split_pagamento(split)
                st.success(f"Split de pagamento criado com sucesso! ID: {resultado['id']}")
    ...
    if st.button("Carregar Splits Recebidos"):
        ...
        status = st.selectbox("Filtrar por Status (opcional)", options=["PENDING", "AWAITING_CREDIT", "CANCELLED", "DONE", "REFUNDED", "BLOCKED_BY_VALUE_DIVERGENCE"], index=0)
    ...
    if st.button("Carregar Splits Pagos"):
        offset = st.number_input("Offset", min_value=0, value=0, key="offset_pagos")
        limit = st.number_input("Limite", min_value=1, max_value=100, value=10, key="limit_pagos")
        paymentId = st.text_input("Filtrar por ID de Cobrança (opcional)", key="paymentId_pagos")
        status = st.selectbox("Filtrar por Status (opcional)", options=["PENDING", "AWAITING_CREDIT", "CANCELLED", "DONE", "REFUNDED", "BLOCKED_BY_VALUE_DIVERGENCE"], index=0, key="status_pagos")
