...
async def fetch_assinaturas():
    async with AsyncClient() as client:
        ...
        return response.json()["data"]  # Retorna apenas os dados das assinaturas

async def show_assinaturas():
    ...
    with st.form(key='create_assinatura'):
        ...
        valor = st.number_input("Valor da Assinatura", min_value=0.0, format="%.2f")
        dueDate = st.date_input("Data de Vencimento")
        format_date = dueDate.strftime("%d/%m/%y")
        billingType = st.selectbox("Tipo de Cobrança", options=["CARTAO", "BOLETO", "DEBITO", "PIX"])
        paymentMethod = st.selectbox("Método de Pagamento", options=["CARTAO", "BOLETO", "DEBITO", "PIX"])
